...
Lab 6 makes the bimodality concrete. The state is $\bar{x} = (x, y)$, the dynamics are known exactly, and the only sensor reports the **distance to the origin**, p(zt​∣xˉt​)=N(∥xˉt​∥,σmeas^2​). Running the filter, the particles cluster in **two** locations.
