...
Lab 6 makes the bimodality concrete. The state is $\bar{x} = (x, y)$, the dynamics are known exactly, and the only sensor reports the **distance to the origin**, $p(z_t \mid \bar{x}_t) = \mathcal{N}(\lVert \bar{x}_t \rVert, \sigma_{\mathrm{meas}}^2)$. Running the filter, the particles cluster in **two** locations.
